...
<form method="POST" action="{{ $action }}" class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6 space-y-6">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        @if ($isEditing)
            <div class="lg:col-span-2 rounded-lg border border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 p-4">
                <label class="flex items-start gap-3">
                    <input id="changePasswordToggle" name="change_password" type="checkbox" value="1"
                        @checked($isChangingPassword)
                        class="mt-1 rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                    <span>
                        <span class="block text-sm font-semibold text-gray-800 dark:text-gray-200">Ubah password</span>
                        <span class="block text-xs text-gray-500 dark:text-gray-400">Aktifkan hanya jika ingin mengganti password user ini.</span>
                    </span>
                </label>
            </div>
        @endif
        ...
        <div>
            ...
            <input id="password" name="password" type="password" autocomplete="new-password"
                ...
                class="mt-1 block w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500">
            @if ($isEditing)
                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Kosongkan jika password tidak diubah.</p>
            @endif
